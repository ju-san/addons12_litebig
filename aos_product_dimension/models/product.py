...
class ProductTemplate(models.Model):
    ...
    product_package_po = fields.Float('Quantity on Cartoon'  ,help="Qty in Package ext:10", digits=dp.get_precision('Product Unit of Measure'))
    ...
    volume_unit = fields.Float(
        string='Volume Unit', digits=dp.get_precision('Volume'), help="The volume in m3.",
        )#compute='_compute_weight_unit', inverse='_set_weight_unit', store=True)
    weight_unit = fields.Float(
        string='Weight Unit', digits=dp.get_precision('Stock Weight'), help="The weight of the contents in Kg, not including any packaging, etc.",
        )
    volume = fields.Float(related='volume_unit',
        string='Volume', digits=dp.get_precision('Volume'), help="The volume in m3.",
        )#compute='_compute_weight_unit', inverse='_set_weight_unit', store=True)
    weight = fields.Float(related='weight_unit',
        string='Weight', digits=dp.get_precision('Stock Weight'), help="The weight of the contents in Kg, not including any packaging, etc.",
        )#compute='_compute_weight_unit', inverse='_set_weight_unit', store=True)
#     weight = fields.Float('Weight (Kg)', compute='_compute_weight_unit', digits=dp.get_precision('Stock Weight'),
...
class ProductProduct(models.Model):
    ...
    package_uom = fields.Many2one('product.packaging', help="Package ex:Dozen, Pack,", readonly=False)
    #package_uom = fields.Char('Package Name',default='Cartoon', help="Package ex:Dozen, Pack,", readonly=True)
    packaging_weight = fields.Float('Packaging Weight', digits=dp.get_precision('Stock Weight'))
    height_dimension = fields.Float('Height', digits=dp.get_precision('Stock Dimension'))
    length_dimension = fields.Float('Length', digits=dp.get_precision('Stock Dimension'))
    width_dimension = fields.Float('Width', digits=dp.get_precision('Stock Dimension'))
    ...
    #weight_unit = fields.Float('Weight(Kg)', digits=dp.get_precision('Stock Weight'))
    size = fields.Float('Volume Cartoon',digits=(10,4))

...
class StockPicking(models.Model):
    ...
    @api.depends('sale_id', 'sale_id.payment_status')
    def _get_payment_status(self):
        for picking in self:
            payment_status = picking.sale_id.payment_status
            if picking.order_policy == 'prepaid' and payment_status == 'paid':
                is_locked = True
            else:
                is_locked = False
            #print ('===is_locked===',picking,is_locked)
            if picking.sale_id:
                picking.write({'is_locked': is_locked})
            picking.update({
